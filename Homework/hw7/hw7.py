...
#              bbb w  x  w  x
# encodes the string 0011011, using runBits=2.
...
def listOfRunLengths(s):
    ...
    def lORL(s, result, curCount, curVal):
        ...
        if s == '':
            ...
            result += [count_bit(s, curVal)]
            curCount += result[-1]
        return lORL(s[curCount:], result, 0, 1 - int(curVal))
        
    return lORL(s[:], [], 0, s[0]) # don't change this line
    # first run begins with s[0] and has length 1 so far
#print(listOfRunLengths(L0))
########
# Step 2
# Complete the following definition.
########
#print(sum(listOfRunLengths(L0))/len(listOfRunLengths(L0)))
def findRunBits(s):
